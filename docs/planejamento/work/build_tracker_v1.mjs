...
const tasks=JSON.parse(await fs.readFile(path.join(base,'work/backlog_v1.json'),'utf8'));
const decisions=JSON.parse(await fs.readFile(path.join(base,'work/decisions_v1.json'),'utf8'));
const scope=JSON.parse(await fs.readFile(path.join(base,'work/scope.json'),'utf8'));
...
backlog.getRange('A3').values=[['Detalhes de objetivo, execução, aceite e impacto: documento MAJUCAU_BACKLOG_DETALHADO_v1.md, pelo ID.']];
...
title(dec,'A2','Majucau — decisões e verificações');
dec.getRange('A3').values=[['Respostas atuais prevalecem apenas nos pontos respondidos; divergências restantes serão decididas caso a caso.']];
...
dec.getRange(`C6:C${dl}`).dataValidation={rule:{type:'list',values:['Em aberto','Respondida','Parcialmente esclarecida','Planejamento autorizado','A verificar tecnicamente']}};
...
summary.getRange('A3').values=[['Versão 1 | 13/09/2026 | Escopo em refinamento | Sem desenvolvimento do produto nesta etapa']];
...
const metrics=[['Tarefas no catálogo',n,'Escopo provisório'],['Levantamentos concluídos',null,'Com evidência'],['Conclusão por peso',null,'Peso inicial = 1'],['Desenvolvimento do produto',null,'Tarefas de desenvolvimento'],['Registros pendentes',null,'Inclui verificações técnicas']];
...
summary.getRange('B10').formulas=[[`=COUNTIFS('Decisoes'!$C$6:$C$${dl},"Em aberto")+COUNTIFS('Decisoes'!$C$6:$C$${dl},"Parcialmente esclarecida")+COUNTIFS('Decisoes'!$C$6:$C$${dl},"A verificar tecnicamente")`]];
...
summary.getRange('A31').values=[['Atualização: consolidação enviada em 13/09/2026. Bling + três estruturas do Drive somente de leitura.']];
...
await fs.mkdir(path.join(base,'work/previews_v1'),{recursive:true});
for(const [name,range] of [['Evolucao','A1:G35'],['Tarefas','A1:G11'],['Decisoes','A69:F74'],['Verificacoes','A84:F88'],['Cobertura','A1:E11']]){
 const png=await wb.render({sheetName:name==='Verificacoes'?'Decisoes':name,range,scale:1,format:'png'});
 await fs.writeFile(path.join(base,'work/previews_v1',name+'.png'),new Uint8Array(await png.arrayBuffer()));
}
const file=await SpreadsheetFile.exportXlsx(wb);
await file.save(path.join(base,'outputs/MAJUCAU_BACKLOG_E_EVOLUCAO_v1.xlsx'));
console.log(JSON.stringify({tasks:n,decisions:decisions.length,summary:summary.getRange('B6:B10').values,output:'MAJUCAU_BACKLOG_E_EVOLUCAO_v1.xlsx'}));
