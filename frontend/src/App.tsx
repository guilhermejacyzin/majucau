...
function IntegrationCard({ integration, bootstrap, blingConfig, nuvemshopConfig, blingOAuth, futurePreview, futureImporter }: { integration: Integration; bootstrap: BootstrapState; blingConfig: BlingConfigAdapter; nuvemshopConfig: NuvemshopConfigAdapter; blingOAuth: BlingOAuthAdapter; futurePreview: NuvemPagoFuturePreviewAdapter; futureImporter: NuvemPagoFutureImportAdapter }) {
  ...
  const disconnect = async () => {
    ...
    try {
      setOauthSessionId('')
      setOauthResult(await blingOAuth.disconnect())
  ...
  const displayStatus = oauthResult?.status === 'CONNECTED' || oauthResult?.status === 'AUTHORIZING' || oauthResult?.status === 'NOT_CONFIGURED' ? oauthResult.status : (liveStatus?.status ?? (integration.key === 'nuvempago' ? 'UNAVAILABLE' : 'NOT_CONFIGURED'))
  ...
  return <article className={`integration-card integration-${integration.key}`}>
    ...
    {integration.fields.length > 0 ? <form onSubmit={(event) => void submit(event)} autoComplete="off" className="integration-form">
      ...
      <div className="integration-actions">{(integration.key === 'bling' || integration.key === 'nuvemshop') && <Tooltip id={integration.key === 'bling' ? 'bling.save' : 'nuvemshop.save'}><button type="submit" className="button primary" disabled={saving} aria-label={integration.key === 'bling' ? 'Salvar configuração' : 'Salvar configuração da Nuvemshop'}>{saving ? 'Salvando…' : 'Salvar configuração'}</button></Tooltip>}<Tooltip id={actionTooltip}><button type="button" className="button primary" onClick={() => void connect()} disabled={integration.key !== 'bling' || oauthBusy || disconnectBusy || displayStatus === 'AUTHORIZING'} aria-disabled={integration.key !== 'bling' || undefined}>{oauthBusy ? 'Abrindo…' : 'Conectar'}</button></Tooltip><Tooltip id="bling.reconnect"><button type="button" className="button ghost" onClick={() => void connect()} disabled={integration.key !== 'bling' || oauthBusy || disconnectBusy || displayStatus !== 'CONNECTED'} aria-disabled={integration.key !== 'bling' || undefined}>Reconectar</button></Tooltip><Tooltip id="action.test"><button type="button" className="button secondary" onClick={() => void testConnection()} disabled={integration.key !== 'bling' || testBusy || disconnectBusy} aria-disabled={integration.key !== 'bling' || undefined}>{testBusy ? 'Testando…' : 'Testar conexão'}</button></Tooltip><Tooltip id="bling.disconnect"><button type="button" className="button ghost" onClick={() => void disconnect()} disabled={integration.key !== 'bling' || disconnectBusy || displayStatus !== 'CONNECTED'} aria-disabled={integration.key !== 'bling' || undefined}>{disconnectBusy ? 'Desconectando…' : 'Desconectar'}</button></Tooltip><Tooltip id="action.syncNow"><button type="button" className="button ghost" disabled={integration.key !== 'bling' || displayStatus !== 'CONNECTED'} aria-expanded={integration.key === 'bling' ? syncOpen : undefined} aria-controls={integration.key === 'bling' ? 'bling-sync-panel' : undefined} onClick={() => setSyncOpen((open) => !open)}>{syncOpen ? 'Fechar sincronização' : 'Sincronizar agora'}</button></Tooltip></div>
    </form> : <><NuvemPagoPricingPanel /><NuvemPagoFuturePanel previewAdapter={futurePreview} importer={futureImporter} /><div className="unavailable-box"><div className="unavailable-icon" aria-hidden="true">◌</div><div><strong>Fonte oficial não disponível</strong><p>Pedidos e status não comprovam taxa, líquido ou data prevista. O valor financeiro permanece <em>UNAVAILABLE</em> até existir uma fonte oficial.</p><Tooltip id="nuvempago.partial"><span className="text-link muted-link">Por que isso importa?</span></Tooltip></div></div></>}
  </article>
}
...
function NuvemPagoFuturePanel({ previewAdapter, importer }: { previewAdapter: NuvemPagoFuturePreviewAdapter; importer: NuvemPagoFutureImportAdapter }) {
  const [folder, setFolder] = useState('')
  const [preview, setPreview] = useState<Awaited<ReturnType<NuvemPagoFuturePreviewAdapter['preview']>> | null>(null)
  const [result, setResult] = useState<Awaited<ReturnType<NuvemPagoFutureImportAdapter['import']>> | null>(null)
  const [busy, setBusy] = useState(false)
  const validate = async () => { if (!folder.trim()) return; setBusy(true); try { setPreview(await previewAdapter.preview(folder.trim())) } finally { setBusy(false) } }
  const importFolder = async () => { if (!folder.trim() || !preview || preview.receivable_count === 0) return; setBusy(true); try { setResult(await importer.import(folder.trim())) } finally { setBusy(false) } }
  return <section className="folder-import-panel" aria-label="Importação de recebimentos futuros do Nuvem Pago"><div className="folder-import-heading"><strong>Importar recebimentos futuros B2C</strong><Tooltip id="nuvempago.futureFolder"><span className="help-dot">?</span></Tooltip></div><p>Use somente a pasta controlada 02_nuvem_pago\recebimentos_futuros; estes dados ficam projetados e não dão baixa em recebidos.</p><div className="folder-import-controls"><Tooltip id="field.value"><input aria-label="Pasta dos recebimentos futuros" value={folder} onChange={(event) => { setFolder(event.target.value); setPreview(null); setResult(null) }} placeholder="C:\\Majucau\\02_nuvem_pago\\recebimentos_futuros" /></Tooltip><Tooltip id="nuvempago.futurePreview"><button type="button" className="button secondary" onClick={() => void validate()} disabled={busy || !folder.trim()}>{busy ? 'Validando…' : 'Validar futuros'}</button></Tooltip>{preview && !preview.error_code && preview.receivable_count > 0 && <Tooltip id="nuvempago.futureImport"><button type="button" className="button primary" onClick={() => void importFolder()} disabled={busy}>{busy ? 'Gravando…' : 'Importar futuros'}</button></Tooltip>}</div>{preview?.error_code ? <div className="folder-import-error" role="status"><strong>{preview.error_code}</strong><span>{preview.message}</span></div> : preview ? <div className="folder-import-result" role="status"><strong>{preview.receivable_count} recebível(is) futuro(s) válido(s)</strong><span>{preview.files.length} arquivo(s) · {preview.error_count} linha(s) com atenção · {preview.ignored_count} ignorado(s)</span>{preview.issues && preview.issues.length > 0 && <ul>{preview.issues.slice(0, 3).map((issue, index) => <li key={`${issue.file}-${issue.line}-${index}`}>{issue.file}{issue.line > 0 ? ` · linha ${issue.line}` : ''}: {issue.message}</li>)}</ul>}</div> : null}{result && <div className={result.error_code ? 'folder-import-error' : 'folder-import-result'} role="status"><strong>{result.error_code ?? `Lote ${result.status}`}</strong><span>{result.message ?? `${result.records_created} criado(s), ${result.records_updated} atualizado(s), ${result.records_failed} com erro.`}</span></div>}</section>
}

function IntegrationsView({ bootstrap, blingPreview, blingImporter, futurePreview, futureImporter, blingConfig, nuvemshopConfig, blingOAuth }: { bootstrap: BootstrapState; blingPreview: BlingPreviewAdapter; blingImporter: BlingImportAdapter; futurePreview: NuvemPagoFuturePreviewAdapter; futureImporter: NuvemPagoFutureImportAdapter; blingConfig: BlingConfigAdapter; nuvemshopConfig: NuvemshopConfigAdapter; blingOAuth: BlingOAuthAdapter }) {
  return <div className="page-content"><div className="page-heading"><div><p className="eyebrow">CONFIGURAÇÕES</p><h1>Integrações</h1><p className="heading-copy">Conecte fontes oficiais para habilitar os dados do painel.</p></div><span className="security-note"><span aria-hidden="true">⌁</span> Credenciais protegidas pelo Windows</span></div><section className="notice notice-warning" aria-label="Aviso de segurança"><span className="notice-icon" aria-hidden="true">!</span><div><strong>Autorização sempre no navegador externo</strong><p>O login não é capturado nesta interface. O Client Secret digitado é transitório e será enviado somente ao worker local; ele não fica em estado, log ou armazenamento do frontend.</p></div></section>{bootstrap.error_code && <section className="notice notice-error" aria-label="Estado do worker"><span className="notice-icon" aria-hidden="true">!</span><div><strong>{bootstrap.error_code}</strong><p>{bootstrap.message}</p></div></section>}<div className="integration-list">{integrations.map((integration) => <div key={integration.key}><IntegrationCard integration={integration} bootstrap={bootstrap} blingConfig={blingConfig} nuvemshopConfig={nuvemshopConfig} blingOAuth={blingOAuth} futurePreview={futurePreview} futureImporter={futureImporter} />{integration.key === 'bling' && <BlingImportPanel adapter={blingPreview} importer={blingImporter} />}</div>)}</div></div>
}
...
function App({ bootstrapAdapter, dashboardSnapshotAdapter, blingPreviewAdapter, blingImportAdapter, futurePreviewAdapter, futureImportAdapter, blingConfigAdapter, nuvemshopConfigAdapter, blingOAuthAdapter }: AppProps = {}) {
  ...
  const futureImporter = useMemo(() => futureImportAdapter ?? createNuvemPagoFutureImportAdapter(), [futureImportAdapter])
